...
import { maturationInputSchema } from "@schemas";
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  switch (method) {
    case "POST":
      try {
        const { apiKey, ...input } = maturationInputSchema.parse(req.body);
        ...
        try {
          await controller.createMaturationRead({
            ...input,
          ...
          if (error instanceof UniqueConstraintFailedError) {
            res.status(400).send({
              success: false,
              message: `Ya existe una lectura de la etapa de maduración para la quesería con el rut "${input.dairyRut}" en la fecha y hora "${input.datetime}"`,
            });
            break;
          }
          res.status(500).send({
